...
class Regressor(object):
    ...
    def build(self, out_layer_activation="linear"):
        ...
        if self["optimizer"] == "adam":
            ...
            op = self["optimizer"]
        model.compile(loss=self["loss"], optimizer=op, metrics=self["metrics"])
    ...
    def load(self, name, custom_objects=None, verbose=True, path="./model/"):
        ...
        self.kwargs["model"] = load_model(path + name + ".h5", custom_objects=custom_objects)
        # load history
        self.kwargs["history"] = eval(json.load(open(path + name + "_history.json", 'r')))
        if verbose:
            print(" done!")
